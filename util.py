...
class SQuAD(data.Dataset):
    ...
    def __init__(self, data_path, use_v2=True):
        ...
        size = 30000
        self.context_idxs = torch.from_numpy(dataset['context_idxs'][:size]).long() #size * max_para_limit
        
        self.context_char_idxs = torch.from_numpy(dataset['context_char_idxs'][:size]).long()
        self.question_idxs = torch.from_numpy(dataset['ques_idxs'][:size]).long()
        #print("ques_idx shape",self.question_idxs.shape)
  
        self.question_char_idxs = torch.from_numpy(dataset['ques_char_idxs'][:size]).long()
        self.y1s = torch.from_numpy(dataset['y1s'][:size]).long()
        self.y2s = torch.from_numpy(dataset['y2s'][:size]).long()

        if use_v2:
            # SQuAD 2.0: Use index 0 for no-answer token (token 1 = OOV)
            batch_size, c_len, w_len = self.context_char_idxs.size()
            ones = torch.ones((batch_size, 1), dtype=torch.int64) 
            self.context_idxs = torch.cat((ones, self.context_idxs), dim=1) #prepending a column of 1s 
            self.question_idxs = torch.cat((ones, self.question_idxs), dim=1) #prepending a column of 1s

            ones = torch.ones((batch_size, 1, w_len), dtype=torch.int64)
            self.context_char_idxs = torch.cat((ones, self.context_char_idxs), dim=1) #prepending a row of 1 for each token 
            self.question_char_idxs = torch.cat((ones, self.question_char_idxs), dim=1)

            self.y1s += 1
            self.y2s += 1 #incrementing all ans idxs by 1
        # SQuAD 1.1: Ignore no-answer examples
        self.ids = torch.from_numpy(dataset['ids'][:size]).long()
        self.valid_idxs = [idx for idx in range(len(self.ids))
                           if use_v2 or self.y1s[idx].item() >= 0]

    def __getitem__(self, idx):
        idx = self.valid_idxs[idx] # get a valid idx from valid_idxs
        example = (self.context_idxs[idx],
                   self.context_char_idxs[idx],
                   self.question_idxs[idx],
                   self.question_char_idxs[idx],
                   self.y1s[idx], # oneidx per example
                   self.y2s[idx], # one idx per example
                   self.ids[idx]) # one idx per example
        # context_idxs[idx] shape is para_limit+1 coz we prepended 1 
        #print("ques idx shape --- ",self.question_idxs[idx].shape)
        #print("**",self.context_idxs[idx].shape,self.question_idxs[idx].shape,"**")
        return example

    def __len__(self):
        return len(self.valid_idxs) #total number of valid idxs
